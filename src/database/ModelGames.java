...
public class ModelGames {
	...
	try {
		...
		query = "SELECT name, ghost, ROUND(((time_played / 60)/60),2), times, category.name_category, completed, score, path FROM `games` INNER JOIN category ON category.id = games.category WHERE completed = "
			+ comp + " ORDER BY name";
	    } else if (name != "Todos") {
		query = "SELECT name, ghost, ROUND(((time_played / 60)/60),2), times, category.name_category, completed, score, path FROM `games` INNER JOIN category ON category.id = games.category WHERE name = '"
			+ name + "'";
	    } else if (category != "Todos" && comp == 2) {
		query = "SELECT name, ghost, ROUND(((time_played / 60)/60),2), times, category, completed, score, path FROM games WHERE category = "
	...
	try {
	    ...
	    if (r == 0) {
		JOptionPane.showMessageDialog(null, "No se ha podido sumar el tiempo jugado",
			"Error al guardar los datos", JOptionPane.ERROR_MESSAGE);
	    }
	    p.close();
	    conex.close();
    ...
    public int editGame(int gameId, String name, int secondsPlayed, String path, String ghost, String times,
	    String completed, int score, int category, int hidden) {
	try {
	    String query = "UPDATE games SET name = ?, time_played = ?, path = ?, ghost = ?, times = ?, completed = ?, score = ?, category = ?, hidden = ? WHERE id = ?";
	    conex = DriverManager.getConnection(Data.url, Data.username, Data.password);
	    PreparedStatement p = conex.prepareStatement(query);
	    p.setString(1, name);
	    ...
	    Log.Loguear(
		    "SQLException en int ModelGames.editGame(int gameId, String name, String MinsPlayed, String path, String ghost)");
	    ex.printStackTrace();
	}
	return 0;
    }
